#!/bin/bash
# Gate attempt 4: wait for holdout24 to finish, package + upload corpora 21-24,
# launch four parallel cloud sweeps (spot-capacity fallback), report verdicts.
# NOTE: code.tar.gz is NOT re-tarred here — a clean snapshot of the committed
# state was uploaded before Track B started editing poke-engine.
...
echo "waiting for holdout24..."
for i in $(seq 1 90); do
  N=$(find /Users/sallyliu/pokemon-ai/synthetic-corpus-holdout24 -name 'battle-*.log' 2>/dev/null | wc -l | tr -d ' ')
  [ "$N" -ge 10000 ] && break
  sleep 60
done
echo "corpora ready"
